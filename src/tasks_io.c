...
// Allocates a region of size 's' and appends it as param to the list of params.
...
// Creates a new parameter (without allocating a memory region)
// and appends it to the list of params.
...
void *attach_output(task_t *t, size_t s)
{
    ...
    if (active_task != NULL) {
        ...
        attach_param_with_elem(
            &active_task->tstate.output_from_dependencies_list, param->elem
        );
        return param->elem;
    } else
        return param->elem;
...
void *retrieve_output_from_dependencies(task_t *t)
{
    ...
    task_param_t *param = retrieve_param(
        &t->tstate.output_from_dependencies_list
    );
    return param->elem;

}
#else
void *retrieve_output_from_dependencies(task_t *t) { return NULL; }
#endif
