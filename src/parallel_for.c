...
void parallel_for_with_reduction(task_routine_t f,
    long int begin_index, long int end_index, long int incr,
    long int nb_blocks, void *output, size_t output_size, reduce_function_t rf)
{

    long int range = end_index - begin_index;

    if (range <= 0 || range % incr != 0 || range / incr % nb_blocks != 0) {
        fprintf(stderr, "ERROR: Not correctly formed parallel loop "
            "-- start: %ld  end: %ld  incr: %ld  blocks: %ld",
            begin_index, end_index, incr, nb_blocks
        );
        exit(EXIT_FAILURE);
    }
